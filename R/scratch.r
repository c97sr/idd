if (false) {
    ...
    ## We try different combination of x which follow multinomial distribution
    ## using rdirichlet. We run all sample space by setting a very large n in rdiric## hlet. ## function and run
    ...
    min_error <- function(y,n){
        ...
        for(counter in 1:n) {
            temp[counter] <- sum((y-rtn[counter,]%*%M)*(y-rtn[counter,]%*%M)) 
        }
      
        rtn[which(temp == min(abs(temp))),]
